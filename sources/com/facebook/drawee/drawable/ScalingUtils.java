...
public class ScalingUtils {
    ...
    private static class ScaleTypeFitEnd extends AbstractScaleType {
        ...
        public void getTransformImpl(Matrix matrix, Rect rect, int i, int i2, float f, float f2, float f3, float f4) {
            ...
            float height = rect.top + (rect.height() - (i2 * min));
            matrix.setScale(min, min);
            matrix.postTranslate((int) (rect.left + (rect.width() - (i * min)) + 0.5f), (int) (height + 0.5f));
        }
    }
    ...
    private static class ScaleTypeFocusCrop extends AbstractScaleType {
        ...
        public void getTransformImpl(Matrix matrix, Rect rect, int i, int i2, float f, float f2, float f3, float f4) {
            ...
            if (f4 > f3) {
                float f6 = i * f4;
                float max2 = rect.left + Math.max(Math.min((rect.width() * 0.5f) - (f * f6), 0.0f), rect.width() - f6);
                max = rect.top;
                f5 = max2;
                f3 = f4;
            } else {
